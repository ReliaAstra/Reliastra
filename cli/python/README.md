...
fails with that message instead of guessing:
`go install github.com/ReliaAstra/Reliastra/cli/cmd/reliastra@latest`.

Upgrading is `pipx upgrade reliastra` (or
`pip install --upgrade reliastra`); each version caches its own binary, so
an upgrade downloads once. Uninstalling is `pipx uninstall reliastra` plus
`rm -rf ~/.cache/reliastra` (Linux) if you want the binaries gone.
