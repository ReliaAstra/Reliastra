...
- Direct answer pattern: ${SITE_URL}/down/{vendor} - the answer to "is {vendor} down?", composed from the same record read and the same answer functions as the vendor record's masthead, so the two surfaces state one answer. Endpoint scoped; a stale or absent observation publishes "unknown" rather than an old state; a category slug or an untracked name 404s; a failed read serves 5xx rather than an answer. JSON twin: ${SITE_URL}/down/{vendor}/index.json (noindex; carries the question, the lead answer, the measured facts, the caveats and the freshest observation's own timestamp).
- Feeds (RSS 2.0): ${SITE_URL}/observatory/incidents/feed.xml - cross-vendor detected incidents (guid = incident id, description = the exact measurement statement). ${SITE_URL}/observatory/{vendor}/incidents/feed.xml - the same items for one vendor (404 for untracked names). ${SITE_URL}/observatory/catalog.xml - tracked-dependency announcements (guid = vendor slug, pubDate = the catalog row's own date or none). An unreadable API degrades every feed to a valid empty document, never a 5xx.
