...
export function EmptyState({ icon, title, body, actionLabel, onAction, helpLabel, onHelp }: Props) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-rs-border-subtle bg-rs-elevated px-6 py-16 text-center">
      ...
      {helpLabel && onHelp && (
        <button type="button" onClick={onHelp} className="mt-3 text-sm text-rs-text-accent hover:underline">
          {helpLabel}
        </button>
      )}
    </div>
