...
interface Props {
  status: string;
  /** When true, pulse animation is disabled (for historical rows per spec) */
...
export function StatusBadge({ status, disablePulse = false, label, className }: Props) {
  ...
  return (
    <span
      className={cn(
        // Spec: rounded-full px-2 py-0.5 text-[11px] font-medium, optional 6px dot
