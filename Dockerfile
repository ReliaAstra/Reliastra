...
COPY --from=frontend-builder /build/.next/standalone /app/web/
...
# Playwright Chromium for evidence-generation tasks (best effort, mirrors
# backend/Dockerfile behaviour).
...
# Process supervision for the whole stack
