...
* it is timestamped and paired with the disclaimer from
  ``app.modules.billing.pricing``, which states plainly that the charge is the
  USD price converted at this rate;
* it **prices the charge**. ``app.modules.billing.pricing`` still never imports
...
async def _cache_store(payload_json: str, ttl: int) -> None:
    ...
    from app.platform.integrations.redis import safe_redis_set
...
async def _cache_read() -> str | None:
    from app.platform.integrations.redis import safe_redis_get
