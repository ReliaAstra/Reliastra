...
``PRODUCT PRICING``
    ...
    in :mod:`app.platform.commercial.entitlements` (``PLAN_PRICES_USD`` /
...
* **The rate is always an explicit input.** Nothing here fetches or caches a
  ...
  through :func:`app.modules.billing.disclosure.resolve_payment_price_async` (or
  :func:`app.modules.billing.fx_reference.current_rate`) and pass it in, so the figure
