...
@celery_app.task(name="app.modules.email_events.tasks.process_resend_event", autoretry_for=(Exception,), max_retries=3, retry_backoff=True)
def process_resend_event(event_db_id: str):
    async def _run(session):
        ...
        res = await session.execute(select(ResendWebhookEvent).where(ResendWebhookEvent.id == uuid.UUID(event_db_id)))
        ...
        try:
            ...
            q = await session.execute(select(func.count(E.id)).where(E.event_type == "email.bounced", E.created_at >= since))
