...
from app.modules.email_events.models import EmailRecord, ResendWebhookEvent, EmailSuppression

logger = logging.getLogger(__name__)

# State ranking for out-of-order handling: higher = more terminal
STATE_RANK = {
    "created": 0, "sending": 1, "sent": 2, "scheduled": 2,
    "delivery_delayed": 3, "delayed": 3,
    "delivered": 4,
    "bounced": 5, "failed": 5, "suppressed": 5, "complained": 5,
    "opened": 3, "clicked": 3, "received": 3,
}
...
class ResendWebhookService:
    async def verify_signature(self, raw: bytes, svix_id: str | None, svix_timestamp: str | None, svix_signature: str | None) -> bool:
        secret = settings.RESEND_WEBHOOK_SECRET.get_secret_value() if settings.RESEND_WEBHOOK_SECRET else None
        if not secret:
    ...
    async def accept_event(self, db: AsyncSession, payload: dict[str, Any], svix_id: str | None) -> str:
        ...
        event_id = svix_id or str(payload.get("id") or data.get("email_id") or "") or f"unknown-{hash(json.dumps(payload, sort_keys=True))}"
    ...
    async def _apply_state(self, db: AsyncSession, evt: ResendWebhookEvent):
        ...
        m = {
            "email.sent": "sent", "email.sending": "sending", "email.scheduled": "scheduled",
        ...
        if not evt.resend_email_id:
            return
        rec = await db.execute(select(EmailRecord).where(EmailRecord.resend_id == evt.resend_email_id))
            ...
            if _rank(status) >= _rank(email.status):
                ...
                if evt.payload:
                    ...
                    if "bounce" in evt.event_type:
                        meta["bounce"] = {k: data.get(k) for k in ("bounce_type","bounce_subtype","message") if k in data}
        ...
        if evt.event_type in ("email.bounced", "email.complained", "email.suppressed") and evt.recipient:
            ...
            if is_permanent:
                # Upsert suppression
                existing = await db.execute(select(EmailSuppression).where(EmailSuppression.recipient == evt.recipient))
                sup = existing.scalar_one_or_none()
                if not sup:
                    sup = EmailSuppression(recipient=evt.recipient, reason=evt.event_type.split(".")[-1], last_event_id=evt.event_id, meta={"resend_email_id": evt.resend_email_id})
                    db.add(sup)
                else:
                ...
                logger.warning("email suppression %s for %s id=%s", evt.event_type, evt.recipient[:3]+"***", evt.resend_email_id)
