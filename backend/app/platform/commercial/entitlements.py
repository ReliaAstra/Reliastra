...
Canonical home (moved from ``app.core.permissions``).

Why platform, not modules/billing? Entitlements are imported by the auth layer
(``app.api.deps``) and eight other domains. Domain packages execute their
``__init__`` (which mounts routers) on any submodule import, so a shared
kernel owned by a domain package creates import cycles. Shared kernels live
in ``app.platform``; ``app.modules.billing`` owns transactions/checkout. This is billing-
