...
async def test_quote_is_the_render_model_and_offers_card_only(
    ...
    assert quote["product_price_display"] == "$9.00 (USD)"
    assert quote["payment_amount_display"] == "\u20a611,898.00 (NGN)"
...
async def test_fx_reference_rates_the_charge(
    ...
    # The charge is exactly that conversion: 900 cents x 1650 = ₦14,850.00.
    assert quote["payment_amount_minor"] == 1_485_000
    assert quote["payment_amount_display"] == "\u20a614,850.00 (NGN)"
...
@pytest.mark.asyncio
async def test_annual_quote_is_not_offered(async_client, auth_data):
    """Monthly is the only interval: an annual quote is unavailable and
    invents neither a product price nor a charge."""
    res = await async_client.get(
...
async def test_quote_refuses_to_offer_checkout_it_cannot_honour(
    ...
    assert quote["unavailable_reason"] == CheckoutReason.PRICE_NOT_CONFIGURED
    assert quote["payment_amount_display"] is None
    assert "cannot price this plan" in quote["unavailable_message"]
    assert "$9.00 (USD)" in quote["unavailable_message"]
    assert quote["product_price_display"] == "$9.00 (USD)"
...
async def test_metadata_arriving_as_a_json_string_still_provisions(
    ...
    payload = _paystack_verify_success(reference=reference, amount=1_189_800)
    ...
    plan = await async_client.get("/v1/billing/plan", headers=auth_data["headers"])
    assert plan.json()["plan"] == "pro"
    assert plan.json()["billing_interval"] == "monthly"
