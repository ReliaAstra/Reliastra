...
@pytest.mark.asyncio
async def test_generate_for_incident_no_check_history_defaults_uptime():
    evidence_repository = MagicMock()
    incident_repository = MagicMock()
    snapshot_repository = MagicMock()
    incident_id = uuid.uuid4()
    ...
    service._html_to_pdf = AsyncMock(return_value=b"immutable-pdf")
    service._render_html = AsyncMock(return_value="<html></html>")
    ...
    with (
        ...
        result = await service.generate_for_incident(AsyncMock(), incident_id)

    assert result.checksum == "abcd123456"
    service._render_html.assert_awaited_once()
    render_kwargs = service._render_html.await_args.kwargs
    assert render_kwargs["uptime_pct"] == 100.0
    assert render_kwargs["sla_impact_pct"] == 0.0
